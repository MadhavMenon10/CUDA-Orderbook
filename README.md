...
A hash table insert treats either slot as an empty slot available to claim, while a lookup must treat `UINT64_MAX` as a stopping point as if the desired lookup order was in the table, it should exist at that index (either via linear probing or at the direct hashed index). 
