...
void SoaArrays::reserve(const size_t message_count) {
    ...
    CUDAUtils::check_cuda_error(cudaMallocHost(reinterpret_cast<void**>(&order_ids_), sizeof(std::uint64_t) * message_count), "Memory allocation for order_ids_");
    CUDAUtils::check_cuda_error(cudaMallocHost(reinterpret_cast<void**>(&old_order_ids_), sizeof(std::uint64_t) * message_count), "Memory allocation for old_order_ids_");
    CUDAUtils::check_cuda_error(cudaMallocHost(reinterpret_cast<void**>(&timestamps_), sizeof(std::uint64_t) * message_count), "Memory allocation for timestamps_");
    CUDAUtils::check_cuda_error(cudaMallocHost(reinterpret_cast<void**>(&prices_), sizeof(std::uint32_t) * message_count), "Memory allocation for prices_");
    CUDAUtils::check_cuda_error(cudaMallocHost(reinterpret_cast<void**>(&quantities_), sizeof(std::uint32_t) * message_count), "Memory allocation for quantities_");
    CUDAUtils::check_cuda_error(cudaMallocHost(reinterpret_cast<void**>(&symbol_ids_), sizeof(std::uint16_t) * message_count), "Memory allocation for symbol_ids_");
    CUDAUtils::check_cuda_error(cudaMallocHost(reinterpret_cast<void**>(&order_types_), sizeof(MessageType) * message_count), "Memory allocation for order_types_");
    CUDAUtils::check_cuda_error(cudaMallocHost(reinterpret_cast<void**>(&sides_), sizeof(char) * message_count), "Memory allocation for sides_");
    size_ = 0;
